...
const ArticlesScreen = ({ route: { params } }) => {
  ...
  const { src } = params;
  const { errorMessage, loading, data, totalResults, page } = useSelector(
    ({ articles }) => articles,
  );
  ...
  if (errorMessage) {
    ...
    return (
      <SafeAreaView style={container}>
        <View style={mainView}>
          <Text style={title}>Articles from {src}</Text>
          <Text>
            ({dispayResults} /{totalResults})
          </Text>
        </View>
        <View style={listView}>
          <FlatList data={data} keyExtractor={_keyExtractor} renderItem={_renderRow} />
        </View>
